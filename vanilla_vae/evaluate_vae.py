...
from vanilla_vae.train_vae import test_reconstruction
...
def main(eval_args):
    ...
    if eval_args.eval_mode == 'evaluate':
        ...
        if eval_args.eval_on_train:
            logging.info('Using the training data for eval.')
            valid_queue = train_queue
        args.num_process_per_node, args.num_proc_node = 1, 1  # evaluate only one 1 node
        test_reconstruction(valid_queue, model, 2, eval_args)
        logging.info('reconstruction saved')
        ...
        with torch.no_grad():
            ...
            set_bn(model, bn_eval_mode, num_samples=num_samples, t=eval_args.temp, iter=5000)
            for ind in range(num_iter):  # sampling is repeated.
                ...
                if visualize:
                    ...
                    im.save(os.path.join(eval_args.save, 'generated/samples_%d_temp_%d.png' % (
                    ind, eval_args.temp)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save', type=str, default='/usr/local/data/nimafh/NVAE-Ablation/imgs/',
                        help='location of the checkpoint')
    parser.add_argument('--eval_mode', type=str, default='sample', choices=['sample', 'evaluate', 'evaluate_fid', 'reconstruct'],
